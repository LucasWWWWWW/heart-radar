...
一个本地纯静态 Web 应用,包含两个核心功能:

1. **关系雷达**
   - 输入对方 6 维表现,实时算关心指数 + 阶段判断
   - **健康关系派**规则库默认建议(完全离线)
   - 可选 **🤖 AI 个性化建议**(Claude Haiku 4.5,自填 API key 存浏览器 localStorage)
   - 历史快照 + 折线图(本地保存,不上传)
2. **穿搭模拟器** — 选外貌特征 + 发型/上衣/下装/配饰
   - **2D 简图** — 纯 SVG 实时合成,可导出 PNG
   - **3D 模型** — Three.js 程序化几何体 + **骨骼分组**(armL/R Group, legL/R Group),衣袖跟手臂走
     - 4 个姿态:**站立 / 挥手 / 叉腰 / 走路**
     - 4 个表情:**平静 / 微笑 / 大笑 / 害羞**
     - 自动呼吸 + 眨眼 + 闲置自动旋转 + 鼠标拖拽
   - **AI 真实试衣** — 占位,后续接 Replicate IDM-VTON + 本地 nsfwjs 预检
